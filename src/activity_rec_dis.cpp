  ...
  while(ar.nh_.ok())
      ...
      if(new_msg_flat==true)
	{
	  ...
	  for(int i=0; i<transform_v.size(); i++)
	    {
              double norm = ar.v_norm(transform_v[i]);
              if(ar.current_frame < ar.frame_length-1)
                ar.sk_data->setValue(i,ar.current_frame,norm); //insert one frame (24 points)
              if(ar.current_frame == ar.frame_length-1)
                {
                  ar.sk_data->setValue(i,ar.current_frame,norm);  
                }
