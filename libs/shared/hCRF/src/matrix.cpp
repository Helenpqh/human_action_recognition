...
template <class elType>
void Matrix<elType>::slid_window(int w, int h, elType value)
{
	if(w<=0 || h<=0){
		throw std::invalid_argument("Size must be positive");
	}
	...
	Matrix<elType> tmpMatrix(w,h,value);
		...
		for(int col=0;col<min_w;col++) 
		  memcpy(tmpMatrix.get() + col*h, pData + (col+1)*height, min_h*sizeof(elType));
...
template void Matrix<unsigned char>::slid_window(int w, int h, unsigned char value);
template void Matrix<uint64_t>::slid_window(int w, int h, uint64_t value);
template void Matrix<unsigned int>::slid_window(int w, int h, unsigned int value);
template void Matrix<int>::slid_window(int w, int h, int value);
template void Matrix<float>::slid_window(int w, int h, float value);
...
template void Matrix<double>::slid_window(int w, int h, float value);
